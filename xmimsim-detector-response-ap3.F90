...
MODULE xmimsim_detector_response_ap3
...
REAL (C_DOUBLE), PARAMETER :: grid_thickness = 0.05456 ! cm
...
#endif
          CALL xmi_detector_convolute_history_custom(&
            inputFPtr,&
            var_red_historyPtr,&
            options)
        ENDIF
...
channels_convPtr, options, escape_ratiosCPtr, n_interactions&
        ...
        DO i=0,inputF%detector%nchannels-1
                ...
                !ENDDO
                det_corr = 0.77_C_DOUBLE + 0.23_C_DOUBLE * EXP(-1.0_C_DOUBLE * &
                CS_Total_Kissel(14, i*inputF%detector%gain+inputF%detector%zero) * &
                ElementDensity(14) * grid_thickness)
...
SUBROUTINE xmi_detector_convolute_history_custom(&
        inputFPtr,&
        historyPtr,&
        options)
        IMPLICIT NONE
            ...
            DO k=1,100
              ...
              IF (counts > 0.0_C_DOUBLE) THEN
                ...
                IF (line_energy > 0.0_C_DOUBLE) THEN
                  ...
                  !ENDDO
                  det_corr = 0.77_C_DOUBLE + 0.23_C_DOUBLE * EXP(-1.0_C_DOUBLE * &
                  CS_Total_Kissel(14, line_energy) * ElementDensity(14) * grid_thickness)
...
ENDSUBROUTINE xmi_detector_convolute_history_custom

ENDMODULE xmimsim_detector_response_ap3
